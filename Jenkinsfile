  ...
  stage('Build WAR') {
    ...
    stage("Deploy on Docker Host") {
      steps {
        sshagent(credentials: ['DOCKER_HOST_SSH_KEY_ID']) {
          sh '''#!/bin/bash
            ssh -o StrictHostKeyChecking=no ${DOCKER_USER}@${DOCKER_HOST} APP_NAME="${APP_NAME}" BUILD_NUMBER="${BUILD_NUMBER}" REMOTE_DIR="${REMOTE_DIR}" HOST_PORT="${HOST_PORT}" CONT_PORT="${CONT_PORT}" bash -se <<- _EOF_
              set -e
              cd ${REMOTE_DIR}
              mkdir -p war_hist
